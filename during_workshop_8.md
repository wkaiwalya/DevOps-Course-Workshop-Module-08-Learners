...
Currently we'll deploy every time a change is pushed to the main branch. However you might want to have more control over when deployments happen. Modify your Heroku and workflow setup so you can manually trigger a workflow that promotes a version to the production environment.
